...
export default function DeleteFileModal({ fileId }) {
    const { openDeleteFileModal, setOpenDeleteFileModal } = useContext(ActionsContext);
    const [datas, setDatas] = useState({});

    const DeleteFileHandler = async () => {
        const fileRef = doc(store, "folders", fileId);
        const fileSnap = await getDoc(fileRef);
        if (fileSnap.exists()) {
            setDatas(fileSnap.data());
        }
    }
    ...
    return (
        <Transition show={openDeleteFileModal}>
            <Dialog className="relative z-10" onClose={setOpenDeleteFileModal}>
                <TransitionChild
                            ...
                            <DialogPanel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                                ...
                                <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                                    ...
                                    <button
                                        ...
                                        onClick={() => setOpenDeleteFileModal(false)}
                                        data-autofocus
                                    >
                                        Cancel
                                    </button>
